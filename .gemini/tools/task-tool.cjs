              ...
              id: {
                ...
                properties: {
                  index: {
                    type: 'NUMBER',
                    description:
                      'Required. The 0-based index of the task in the list.',
                  },
                  text_prefix: {
                    type: 'STRING',
                    description:
                      "Required. A prefix of the task's text for verification. Example: 'Buy milk'",
                  },
                },
...
async function updateTasks(params) {
  ...
  if (active_task_id) {
    ...
    if (task.is_complete) {
      throw new Error(
        `Cannot set a complete task as active (index: ${active_task_id.index}).`,
      );
    }
    state.active_task_index = active_task_id.index;
  } else if (
    params.hasOwnProperty('active_task_id') &&
    active_task_id === null
  ) {
    state.active_task_index = null;
  }
...
async function main() {
  ...
  switch (toolName) {
    ...
    default:
      console.error(
        `Unknown tool: ${toolName}. The first argument should be the tool name.`,
      );
      process.exit(1);
  }
}
